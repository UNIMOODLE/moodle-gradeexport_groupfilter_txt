...
class grade_export_groupfilter_xls extends grade_export {
    ...
    public function print_grades() {
        ...
        foreach ($profilefields as $key => $fields){
            if ($profilefields_selected !== null && !in_array($fields->shortname, $profilefields_selected)) unset($profilefields[$key]);
        }
        ...
        if (!empty($groups)){
            ...
            if ($groupid == 0){
                foreach ($groups as $user_group){
                    ...
                    while ($userdata = $gui->next_user()) {
                        ...
                        if (groups_is_member($user_group->id, $user->id)) {
                            ...
                            $myxls->write_string($i, 0, $user_group->name);
                ...
                while ($userdata = $gui->next_user()) {
                    ...
                    //check user is from the group
                    if (groups_is_member($this->groupid, $user->id)) {
